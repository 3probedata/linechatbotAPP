...
#### 一、如何使用 Python LINE Bot SDK 在 Heroku 上架設一個簡單的回話機器人
#### 二、如何進階可以回應圖片等步驟及程序

#### 教案1: LineBotAPP.ipynb setup in Heroku
#### 教案2: Line_image_upload.ipynb setup in Azure CPS(cloud process service)
#### 教案3: Line_Rich_Menu.ipynb
...
4. 關閉預設罐頭回覆訊息- Auto-replay, Greeting等
...
- 架設於Azure的平台，由Hahoku APP去呼叫此功能，用來做圖像辨識的APP。
- 此程式非完整版，僅說明被呼叫時的動作，完整程式需參考[AzureFLaskWeb](https://github.com/3probedata/AzureFlaskWeb)說明

### static/
- 此目錄用於配置圖片
    - error/ # 圖片用於發生錯誤時的回應圖片
    - model/ # 當後端Azure將圖片分析完成後，程式會去取回圖片，並存放在此路徑內
    - RichMenu/ # Setup RichMenu會用到的圖片
